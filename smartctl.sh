...
# 检查是否存在硬件 RAID（megaraid 设备）
if smartctl --scan | grep -q "megaraid"; then
  exclude_sda=true
else
  exclude_sda=false
fi
...
for disk in ${device_list}; do
  ...
  # 排除 /dev/sda 的选项
  if [[ "$device" == "/dev/sda" && "$exclude_sda" == true ]]; then
    echo "Excluding /dev/sda from the check"
    continue
  fi
  ...
  model_name_value=1  # 此处 1 无意义，只是单纯传输，以满足 node-exporter 采集的要求。
  ...
  if [[ "$disk" == sd* ]]; then
    ...
    # Reallocated_Sector_Ct：这是已经重新分配的扇区的数量。如果该值不为0，则磁盘存在坏扇区。正常情况的 RAW_VALUE 为 0
